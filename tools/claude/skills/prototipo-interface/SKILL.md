...
4. **Validar antes de publicar.** Rodar o JS de verdade e conferir erros (arquivos fora do projeto renderizam como snapshot estático — copiar o HTML para dentro da pasta do projeto e testar via Browser pane / `javascript_tool`; localhost é bloqueado no Browser pane). Corrigir bugs antes de entregar o link.
5. **Publicar como Artifact** e mandar o link. Republicar no mesmo arquivo mantém a URL e cria histórico de versões. Artifact nasce privado — compartilhar só via menu "Compartilhar" da página.
...
| Publicar sem testar o JS | Validar rodando no Browser pane antes do link |
